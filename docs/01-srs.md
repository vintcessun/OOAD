...
| IDM-003 | 新增用户 | 系统管理员 | 用户名全局唯一；初始口令强制首次登录修改；新用户默认无任何角色 | 一 |
...
| ROLE-001 | 创建角色 | 系统管理员 | roleCode 全局唯一且不可修改；roleName 可修改 | 一 |
...
| PERM-003 | 为角色授予权限 | 系统管理员 | 幂等；批量操作全成功或全回滚；触发该角色下全部用户的缓存失效；迭代三受 ROLE_PERM_MAX 约束 | 一 |
...
| BIZ-001 | 发文拟稿 | 普通员工 | 受 `oa:doc:draft:create` 权限保护 | 一 |
| BIZ-003 | 查询考勤记录 | 全部 | 受 `hr:attendance:record:view` 保护；迭代二起按数据范围过滤 | 一 |
