...
- [附录 D 与权限清单「系统管理后台」的逐条对照](#附录-d-与权限清单系统管理后台的逐条对照)
...
| 系统名称 | 文档类型 | 子系统 | 顺序编号 |
|---|---|---|---|
| RBAC — 权限管理系统 | REQ：需求文档 | IDM：身份管理<br>ORG：组织架构（部门/岗位）<br>ROLE：角色管理<br>PERM：权限与资源管理<br>HIER：角色继承<br>SCOPE：数据范围<br>CSTR：约束管理<br>AUTHZ：授权服务<br>SEC：安全管理<br>SESS：会话管理<br>MENU：菜单管理<br>AUDIT：审计日志<br>BIZ：受保护业务桩<br>OBSV：监控与可观测性 | 从 001 开始 |
...
| 分级管理 | Delegated Administration | 管理员的管理权限被限定在其组织范围内。如分公司管理员只能管理本分公司及下属部门的用户与角色。**本系统不为此引入新机制——它就是数据范围作用在本系统自身的管理接口上** |
| 授权上界 | Grant Bounding | 管理员只能授予**自己拥有、且范围不超过自己**的权限。防止权限凭空放大与横向越权 |
| 冻结 | Freeze | 账号的第三种状态。暂停但可恢复：保留全部授权关系，但判定一律拒绝、会话失效 |
| 临时人员 | Temporary Personnel | 非正式员工（如外部技术人员）。无工号、无电话、可无所属部门，授权带失效时间 |
| 身份预览 | Impersonation | 管理员以指定用户的身份查看界面，用于验证该用户的实际权限效果。**只读，且全程审计** |
...
### 3.2.2.1 分级管理：数据范围作用于本系统自身

公司需要**分级管理**——分公司要有自己的管理员，大的分公司下面还要有部门管理员，上级管理员管理下级。

一个容易走错的设计是为此新增一套「管理范围」机制（`admin_scope` 字段、`admin_level` 等级）。**本系统不这样做**，理由是：本系统自身的管理功能（用户管理、角色管理、权限配置）也是权限清单中的功能点（`sys:account:user:*`、`sys:account:role:*` 等），既然它们也是受保护的功能点，那么**数据范围机制对它们同样适用**。

于是「分公司管理员」不是一个新概念，只是一次普通的授权：

| 管理员 | 持有权限 | data_scope | 实际能管的范围 |
|---|---|---|---|
| 总公司系统管理员 | `sys:account:user:*` 等 | `ALL` | 全部 10000 人 |
| 道路工程分公司管理员 | 同上 | `DEPT_AND_SUB` | 该分公司及下属 9 个部门共 1120 人 |
| 分公司财务部管理员 | 同上 | `DEPT` | 该部门内的员工 |

管理台自身的每一次写操作，都带 `context.targetDeptId = 目标用户所属部门` 调用同一个 `/authz/check`。**一套机制、一条判定路径**，不存在两份需要保持同步的实现。

**授权上界规则（Grant Bounding）**

仅有范围限制还不够。还必须保证：管理员**只能授予自己拥有、且范围不超过自己的权限**。否则分公司管理员可以给自己的下属授予全公司范围的权限，再通过下属反向获得越权能力。

```
可授予(a, p, s) =
      p ∈ EffectivePermissions(a)          -- 自己得有这条权限
  ∧   s ⊆ DataScope(a, p)                  -- 范围不能超过自己
```

其中范围的偏序为 `SELF ⊂ DEPT ⊂ DEPT_AND_SUB ⊂ ALL`。

> 该规则生效于**授予与指派两个写入口**（`PERM-003`、`ROLE-005`），而不在判定管道中——它约束的是「能不能配」而非「能不能用」。这条区分很重要：把它塞进判定管道会让每次鉴权都多一次无谓的计算，而鉴权的调用量是配置操作的一万倍。

**下级管理员权限被撤销时不级联撤销其已发放的授权**：撤销只是让该管理员失去继续管理的资格，他此前授予给他人的权限保持有效，但标记为「待复核」。理由是级联撤销会造成大面积、不可预期的权限丢失——一个分公司管理员离职，不应导致其下属一千人集体失去权限。
...
| **分公司/部门管理员** | 主要 | ➕ 分级管理 | **不是新角色**，而是持有系统管理权限但 `data_scope` 为 `DEPT_AND_SUB` / `DEPT` 的用户。见 §3.2.2.1 |
| **临时人员** | 主要 | ➕ 我方增设 | 非正式员工（如外部技术人员）。无工号、无电话、可无所属部门；授权带失效时间；**不得持有系统管理权限** |
| 业务系统 | 主要 | — | 18 个业务系统，通过授权接口获取判定结果 |
| **HR 系统** | 主要 | ✅ 清单「人员同步管理」 | 外部系统。通过接口**创建、更新、停用用户**并同步组织架构变更。本系统提供接口，不实现 HR 侧逻辑 |
| 定时任务 | 辅助 | — | 触发临时授权回收、约束存量扫描、冻结到期处理等 |
...
| RBAC-REQ-IDM-009 | HR 系统同步用户 | 用户目标 | 一 | 必须 |
| RBAC-REQ-IDM-010 | 冻结 / 解冻账号 | 用户目标 | 一 | 必须 |
| RBAC-REQ-IDM-011 | 以用户身份预览权限 | 用户目标 | 二 | 必须 |
| RBAC-REQ-IDM-012 | 维护临时人员账号 | 子功能 | 二 | 应该 |
...
| RBAC-REQ-AUDIT-003 | 审计日志防篡改校验 | 子功能 | 二 | 必须 |
| RBAC-REQ-SEC-001 | 配置密码策略 | 用户目标 | 二 | 必须 |
| RBAC-REQ-SEC-002 | 敏感字段加密与脱敏 | 子功能 | 一 | 必须 |
| RBAC-REQ-SEC-003 | 数据字典与编号规则配置 | 子功能 | 三 | 可选 |
| RBAC-REQ-SEC-004 | 数据备份与恢复 | 用户目标 | 三 | 应该 |
| RBAC-REQ-SEC-005 | 接口管理与调用监控 | 用户目标 | 三 | 可选 |
...
| RBAC-REQ-SCOPE-004 | 授权上界校验（分级管理） | 子功能 | 二 | 必须 |
| RBAC-REQ-OBSV-001 | 查看授权监控面板 | 用户目标 | 二 | 应该 |

**迭代一共 36 项（必须级 35 项），构成第一次检查的实现范围。**
...
> **测试系统的规模要求**：老师要求「至少两个系统，每个系统 3–4 个功能」用于测试。因此业务桩取**两个完整模块共 8 个功能点**，而非单个功能点。

**测试系统一：OA 协同办公系统 > 公文管理**（`oa:doc:*`，5 个功能点）

| 功能点 | 权限码前缀 | 系统管理员 | 公司领导 | 部门经理 | 项目经理 | 普通员工 |
|---|---|---|---|---|---|---|
| 收文登记与流转 | `oa:doc:receive` | 全部 | 查看 | 查看/编辑 | 查看 | 查看 |
| **发文拟稿与审核** | `oa:doc:draft` | 全部 | 查看/审批 | 查看/编辑/审批 | 查看 | 查看/新增 |
| 公文签发与归档 | `oa:doc:issue` | 全部 | 查看/审批 | 查看 | 查看 | **无** |
| 公文催办与督办 | `oa:doc:urge` | 全部 | 查看 | 查看/编辑 | 查看 | **无** |
| 密级与权限管理 | `oa:doc:secrecy` | 全部 | 查看 | **无** | **无** | **无** |

**测试系统二：人力资源管理系统 > 考勤与休假管理**（`hr:attendance:*`，3 个功能点）

| 功能点 | 权限码前缀 | 系统管理员 | 公司领导 | 部门经理 | 项目经理 | 普通员工 |
|---|---|---|---|---|---|---|
| **考勤管理** | `hr:attendance:record` | 全部 | 查看 | 查看/编辑 | 查看 | 查看/**编辑本人** |
| 休假管理 | `hr:attendance:leave` | 全部 | 查看 | 查看/编辑 | 查看/新增 | 查看/新增 |
| 加班管理 | `hr:attendance:overtime` | 全部 | 查看 | 查看/编辑 | 查看/新增 | 查看/新增 |

这两个模块的选取不是随意的，它们恰好覆盖了验证权限体系所需的全部情形：

| 需验证的情形 | 覆盖它的功能点 |
|---|---|
| 同一功能点、不同角色有不同操作集 | `oa:doc:draft`（普通员工只能新增，部门经理可审批） |
| 某角色对某功能点**完全无权** | `oa:doc:issue` / `oa:doc:urge` 对普通员工 |
| 仅最高层可见 | `oa:doc:secrecy` 只有系统管理员与公司领导 |
| **数据范围 SELF** | `hr:attendance:record` 普通员工「编辑本人」 |
| 职责分离（拟稿 ≠ 审批） | `oa:doc:draft` |
| 新增类操作的权限区分 | `hr:attendance:leave` / `overtime`（项目经理与普通员工可新增，部门经理只能编辑） |

**桩一重点用例：OA > 公文管理 > 发文拟稿与审核**（`oa:doc:draft:*`）
...
| 用例编号：RBAC-REQ-IDM-009 | 用例名称：HR 系统同步用户 |
|---|---|
| **级别：** 用户目标 ||
| **包：** 身份管理 IDM ||
| **参与者：** HR 系统 ||
| **描述：** 外部 HR 系统通过接口创建、更新、停用用户，并同步组织架构变更。对应权限清单「系统管理后台 > 组织架构同步 > 人员同步管理」。**本系统只提供接口，不实现 HR 侧逻辑** ||
| **触发事件：** 被其他系统调用 ||
| **主成功场景：** | **信息** |
| 1. HR 系统以 `user_type = SYSTEM` 的集成账号调用同步接口 | 用户列表（增/改/停） |
| 2. 系统校验调用方持有 `sys:org:sync:manage` 权限 | |
| 3. 系统按 `username` **upsert**：存在则更新，不存在则创建 | |
| 4. 对于更新：**部门与岗位以 HR 为准**，覆盖本地修改 | |
| 5. 对于停用：将状态置为 `DISABLED`，清理其全部会话，**保留角色指派关系**以备复职 | |
| 6. 系统按 `sys_position_role_mapping` 为新用户派生初始角色 | |
| 7. 系统失效受影响用户的权限缓存 | |
| 8. 系统记录 `USER_SYNC` 审计日志，逐条记录变更 | |
| 9. 系统返回同步结果：新增数、更新数、停用数、失败明细 | |
| **扩展场景：** | **信息** |
| 3a. 用户名冲突（同名不同人） | |
| &nbsp;&nbsp;1. 该条失败并记入失败明细，**其余条目继续** | 批量同步不因单条失败而整体回滚 |
| 4a. 该用户在本系统被手工赋予了额外角色 | |
| &nbsp;&nbsp;1. **保留手工角色**，只同步部门与岗位 | 否则每次同步都会抹掉管理员的手工调整 |
| 5a. 停用的用户是某部门的部门经理 | |
| &nbsp;&nbsp;1. 停用成功，但记入告警：该部门经理岗位空缺 | 部门经理是数据范围的判定依据，空缺会使该部门的 DEPT 范围失去归属 |
| 2a. 调用方无同步权限 | |
| &nbsp;&nbsp;1. 返回 403 | |
| **其他：** 本接口**幂等**：同一批数据重复提交结果一致。组织架构的调整由 HR 侧发起，本系统被动接受——这是「关系改动我们不管权限」的落实方式（待与老师确认，见附录 C.3 Q15） ||

---

### 4.2.18 冻结 / 解冻账号

> 账号有**三种状态**，不是两种。这是本次需求澄清中容易被忽略但影响较大的一条。

| 用例编号：RBAC-REQ-IDM-010 | 用例名称：冻结 / 解冻账号 |
|---|---|
| **级别：** 用户目标 ||
| **包：** 身份管理 IDM ||
| **参与者：** 系统管理员、分公司/部门管理员 ||
| **描述：** 临时暂停某账号的全部权限，但保留其授权关系，以便日后恢复 ||
| **触发事件：** 界面操作 ||
| **主成功场景：** | **信息** |
| 1. 管理员选定用户，选择「冻结」并填写原因 | 用户 ID、原因、可选的自动解冻时间 |
| 2. **系统校验目标用户在管理员的管理范围内**（分级管理） | 见 §3.2.2.1 |
| 3. 系统将用户状态置为 `FROZEN` | |
| 4. 系统**立即失效**其全部会话与权限缓存 | |
| 5. 系统记录 `USER_FREEZE` 审计日志 | 原因、操作人 |
| **扩展场景：** | **信息** |
| 2a. 目标用户不在管理范围内 | |
| &nbsp;&nbsp;1. 返回 `25004 OUT_OF_DATA_SCOPE` | 横向越权拦截 |
| 1a. 目标是最后一个持有 `SYS_ADMIN` 的运维账号 | |
| &nbsp;&nbsp;1. 系统拒绝：不得冻结最后一个管理员 | 否则系统将无人可管 |
| 1b. 设置了自动解冻时间 | |
| &nbsp;&nbsp;1. 定时任务到期后自动解冻并记审计 | |
| **其他：** 见下方三态说明 ||
...
| 状态 | 值 | 语义 | 授权关系 | 会话 | 判定结果 | 典型场景 |
|---|---|---|---|---|---|---|
| 正常 | `ACTIVE`(1) | 可正常使用 | 保留 | 有效 | 按权限判定 | — |
| **冻结** | `FROZEN`(2) | **暂停但可恢复** | **完整保留** | 立即失效 | **一律拒绝** | 休假、调查期间、临时停权 |
| 禁用 | `DISABLED`(0) | 注销 / 离职 | 保留（备查） | 立即失效 | 一律拒绝 | 离职、账号作废 |

> **冻结与禁用的区别不在技术效果，而在语义与可逆性**：两者的判定结果都是拒绝，但冻结预期会被解除，因此界面上应能一键恢复且保留全部配置；禁用是终态，恢复需走重新入职流程。把两者合并成一个 `status = 0` 会丢失这个区分，导致「临时停权」和「离职」在审计日志里无法区分。
>
> 权限本身也可被冻结——即角色-权限授予关系上的冻结标记，效果等同于临时撤销但保留配置。该能力在迭代三随约束体系一并实现。

---

### 4.2.19 以用户身份预览权限

> 本系统的前端是**面向管理员的控制台**，普通员工不在此登录。那么管理员如何验证「许禄到底能看到什么」？答案是身份预览，而不是拿许禄的账号去登录。

| 用例编号：RBAC-REQ-IDM-011 | 用例名称：以用户身份预览权限 |
|---|---|
| **级别：** 用户目标 ||
| **包：** 身份管理 IDM ||
| **参与者：** 系统管理员 ||
| **描述：** 管理员以指定用户的身份渲染界面与调用测试接口，直观查看该用户的实际权限效果。**只读，全程审计** ||
| **触发事件：** 界面操作 ||
| **主成功场景：** | **信息** |
| 1. 管理员在用户详情页点击「以此用户身份查看」 | 目标用户 ID |
| 2. 系统校验管理员持有 `sys:account:user:impersonate` 权限 | |
| 3. **系统校验目标用户在管理员的管理范围内** | |
| 4. 系统签发**短时效**（15 分钟）的预览令牌，载荷中标记 `impersonatedBy` | 预览令牌 |
| 5. 界面进入预览模式，顶部显示醒目横幅「正在以 许禄 的身份预览」 | |
| 6. 预览期间的全部判定**按目标用户的权限执行** | |
| 7. 系统记录 `IMPERSONATE_START` 审计日志，**同时记录操作人与目标** | |
| 8. 管理员退出预览，记录 `IMPERSONATE_END` | |
| **扩展场景：** | **信息** |
| 6a. 预览期间发起写操作 | |
| &nbsp;&nbsp;1. **一律拒绝**。预览模式只读 | 否则无法区分「管理员做的」与「用户做的」，审计将失去意义 |
| 3a. 目标不在管理范围内 | |
| &nbsp;&nbsp;1. 拒绝 | 否则分公司管理员可预览总经理的界面 |
| 2a. 目标持有比管理员更高的权限 | |
| &nbsp;&nbsp;1. 拒绝 | 防止通过预览窥探越权信息 |
| **其他：** 预览令牌不可续期、不可用于调用管理接口。这是「只做管理员界面」这一范围决策的配套设计——没有它，管理员就只能靠读数据库来验证权限配置是否正确 ||
...
老师给出的高峰场景是**上班时段 07:45–08:30**：一万名员工在 45 分钟内陆续登录并开始工作。据此推算：

| 项 | 推算 | 说明 |
|---|---|---|
| 登录请求 | 10,000 ÷ 2,700s ≈ **3.7 次/秒**（均值） | 登录本身压力很小 |
| 鉴权请求（均值） | 假设每人在该窗口内产生 20 次受保护操作<br/>10,000 × 20 ÷ 2,700 ≈ **75 QPS** | |
| 鉴权请求（峰值） | 按 5 倍峰值系数 ≈ **400 QPS** | 上班时间高度集中在 8:00–8:15 |

**实测峰值约 400 QPS，而本文档的目标是 10,000 QPS——为什么留 25 倍余量？**

1. 本系统是**统一授权中心**，服务公司全部 18 个业务系统。上面的推算只覆盖了「人的操作」，而业务系统之间的服务调用同样要鉴权，这部分量级无法从员工数推算；
2. 课程明确要求「高负载大并发」，指标应当体现设计能力而非仅满足当前规模；
3. 400 QPS 用任何实现方式都能满足，无法区分设计优劣；10,000 QPS 才能把缓存策略、判定路径复杂度的差异**压出来**，这正是三范式对比实验需要的区分度。

> 这个推算必须写进文档。检查时若只说「我们的目标是一万 QPS」，老师追问「依据是什么」而答不出，反而是减分项。**有依据的余量是设计，没有依据的数字是口号。**
...
| S-10 | **敏感字段加密存储 + 脱敏展示**：手机号以 AES-256 加密存列，另存脱敏副本 `138****5678` 供列表展示。列表查询一律返回脱敏值，仅在有 `sys:security:data:view` 权限时才可请求明文，且明文访问单独记审计 |
| S-11 | **审计日志防篡改**：每行存 `prev_hash` 与 `row_hash`，`row_hash = SHA256(prev_hash ‖ 本行关键字段)`，构成哈希链。任何对历史行的修改都会使其后全部行的校验失败。提供链完整性校验接口 |
| S-12 | **下级管理员权限被撤销时不级联撤销其已发放的授权**：撤销只使该管理员失去继续管理的资格，其此前授予他人的权限保持有效并标记「待复核」。级联撤销会造成大面积不可预期的权限丢失 |
| S-13 | **授权上界**：管理员只能授予自己拥有、且数据范围不超过自己的权限（见 §3.2.2.1） |
| S-14 | **身份预览只读**：预览令牌短时效（15 分钟）、不可续期、不可发起写操作、不可调用管理接口；预览全程记录操作人与目标 |
| S-15 | 传输一律 HTTPS；口令、令牌、明文手机号不得出现在任何日志或 URL 查询串中 |
| S-16 | **临时人员账号不得持有系统管理权限**，且必须设置失效时间 |

> **双因素认证不在本次范围内**——权限清单的「系统管理后台 > 安全管理 > 双因素认证管理」是清单中的一项，但老师已明确说明本课程设计不要求实现。文档保留该功能点的编号占位，标注「教师明确不要求」，避免检查时被误认为遗漏。
...
| A-4 | 一个用户是否允许拥有多个角色？ | 数据模型按**多对多**建，但老师口径倾向**单角色** | 会议纪要写「用户只可能在一个公司的一个部门，不存在多个角色，但可能后续拓展」。**两种口径都不需要改 schema**：若确认单角色，只需启用 `CARD_USER_ROLE_MAX = 1` 这条基数约束即可。已在 V3 脚本中预置该约束（默认停用） | **待确认** |
| A-4b | 一个用户是否属于多个部门？ | **否，只属于一个部门** | 会议明确：一个用户只属于一个部门，但**可能拥有多个部门的权限**——后者由数据范围表达，不是多部门归属 | 已定 |
...
| A-12 | 权限变更后立即生效还是下次登录生效？ | **立即生效**（≤1 秒）；**但不要求服务端主动推送** | 老师明确「不要求推送，但要求权限一致」。因此**不引入 WebSocket / SSE**：服务端在检测到权限已变更时通过 `X-Permission-Changed` 响应头告知前端刷新，前端另有定时与路由切换两个刷新时机。一致性由缓存失效保证，与推不推送无关 | 已定 |
| A-12b | 一致性语义分档？ | 撤销走**强一致**，新增可**最终一致** | 撤销的不一致窗口是可被利用的安全漏洞；新增的延迟只是用户晚几百毫秒拿到权限 | 已定 |
| A-24 | 前端做到什么范围？ | **只做面向管理员的控制台**；普通员工不在本平台登录 | 会议明确。其他系统的用户在**各自系统**登录，那些系统调用本系统的接口取权限与鉴权。管理员验证他人权限的手段是**身份预览**（IDM-011）与权限模拟器，而不是用别人的账号登录 | 已定 |
| A-25 | 账号有几种状态？ | **三种**：正常 / 冻结 / 禁用 | 会议明确「账号应该有三种状态」。冻结可恢复且保留全部授权，禁用是终态。合并成两态会使「临时停权」与「离职」在审计中无法区分 | 已定 |
| A-26 | 是否存在非员工账号？ | **存在临时人员**：无工号、无电话、可无部门，授权带失效时间，不得持有系统管理权限 | 会议明确「需要准备非员工特例，即临时人员（例如技术人员）」 | 已定 |
| A-27 | 用户由谁创建？ | **HR 系统通过接口创建**；本系统提供接口不实现 HR 逻辑 | 对应权限清单「组织架构同步 > 人员同步管理」。管理台仍保留手工创建入口，用于临时人员 | 已定 |
| A-28 | 分级管理如何实现？ | **复用数据范围**，不新增机制 | 见 §3.2.2.1。本系统自身的管理功能也是受保护功能点，数据范围对其同样适用。新增 `admin_scope` 字段会产生两套需要保持同步的实现 | 已定 |
| A-29 | 测试用的预设系统？ | **两个模块共 8 个功能点**：OA 公文管理（5）+ HR 考勤与休假管理（3） | 老师要求「至少两个系统，每个系统 3–4 个功能」。这两个模块恰好覆盖了验证所需的全部情形，见 §4.2.9 | 已定 |
...
| Q13 | **同级管理员能否相互更改对方权限？** | **未答，且有风险**。会议中提到「同级管理员相互更改对方权限」「自己 同级 以下」。我们默认**否**：两个同级管理员互改会形成提权环路；且在授权上界规则（S-13）下，同级之间谁也不比谁权限多，互改本身也无实际收益。建议明确排除 |
| Q14 | **一个用户是单角色还是多角色？** | **未答**。会议纪要写「不存在多个角色，但可能后续拓展」。数据模型按多对多建，单角色只需启用一条基数约束。见 A-4 |
| Q15 | **「关系改动我们不管权限」具体指什么？** | **未答**。我们的理解是：组织架构调整由 HR 系统发起并同步过来，本系统被动接受，不主动处理调整引发的权限变更。但组织调整**会改变数据范围的计算结果**（部门变了，DEPT 范围覆盖的人就变了），这一点是否也「不管」需要确认 |
| Q16 | **高级管理员管「直接下级」还是「全部下级」？** | **未答**。会议纪要写「高级管理员最好管直接下级」。当前实现为 `DEPT_AND_SUB`（全部下级）。若要求仅直接下级，需新增第五种数据范围 `DEPT_AND_CHILDREN`（仅下一层），改动不大但要提前确认 |
| Q17 | 功能点与接口是一一对应吗？ | **未答**。会议中 Axol 提出此问题未得到答复。当前设计：一个功能点对应一组接口（按操作区分），而非一个接口 |
| Q18 | 权限本身是否可冻结（区别于账号冻结）？ | **未答**。会议提到「存在冻结权限」。当前理解为角色-权限授予关系上的冻结标记，迭代三随约束体系实现 |
...
# 附录 D 与权限清单「系统管理后台」的逐条对照

> 权限清单第 18 个软件系统是「**系统管理后台**」，其 21 个功能点描述的正是**本系统自身**应当具备的能力。换言之，老师在需求数据里已经给出了本课程设计的验收清单。
>
> 本附录逐条对照，确保没有遗漏。**这张表是检查时最有说服力的材料之一**——它证明我们的需求不是自行发挥，而是从老师给的数据中导出的。

| # | 功能模块 | 功能点 | 清单中的功能说明 | 本文档对应需求 | 迭代 |
|---|---|---|---|---|---|
| 1 | 用户与权限管理 | 用户管理 | 账号创建、编辑、停用、删除、密码重置、账号锁定 | IDM-003~007、IDM-010（冻结）、IDM-009（HR 同步） | 一 |
| 2 | 用户与权限管理 | 角色管理 | 角色创建、角色权限配置、角色用户分配、**数据权限设置** | ROLE-001~006、PERM-003、**SCOPE-001** | 一 |
| 3 | 用户与权限管理 | 权限配置 | 功能权限、**数据权限**、按钮权限**分级配置**、**权限继承设置** | PERM-001~005、SCOPE-001~004、**HIER-001~004** | 一 / 二 |
| 4 | 组织架构同步 | 组织架构管理 | **公司、分公司、部门、项目部**组织架构配置、层级调整 | ORG-001~004 | 一 |
| 5 | 组织架构同步 | **人员同步管理** | 与 **HR 系统**人员数据同步、组织架构变更自动同步 | **IDM-009**、ORG-005 | 一 |
| 6 | 组织架构同步 | **数据权限范围配置** | 按组织架构配置用户**数据查看范围**、**数据隔离** | **SCOPE-001~003** | 一（配置）/ 二（生效） |
| 7 | 系统配置管理 | 数据字典管理 | 下拉选项、枚举值、业务字典配置 | SEC-003 | 三（可选） |
| 8 | 系统配置管理 | 系统参数配置 | 业务参数、流程参数、提醒参数配置 | SEC-003 | 三（可选） |
| 9 | 系统配置管理 | 编号规则配置 | 单据编号、合同编号、项目编号规则 | SEC-003 | 三（可选） |
| 10 | 日志审计管理 | 登录日志管理 | 登录时间、IP、设备信息、登录状态 | AUDIT-001（`LOGIN`/`LOGIN_FAILED`）、AUDIT-002 | 一 / 二 |
| 11 | 日志审计管理 | 操作日志管理 | 功能访问、数据修改、操作时间 | AUDIT-001（权限变更类事件）、AUDIT-002 | 一 / 二 |
| 12 | 日志审计管理 | 异常日志管理 | 系统异常、报错日志记录、监控、排查 | AUDIT-001（`SYSTEM_ERROR`）、OBSV-001 | 二 |
| 13 | 系统监控与维护 | 系统性能监控 | CPU、内存、磁盘、**接口响应时间**监控 | **OBSV-001**（鉴权 QPS / P95 / P99） | 二 |
| 14 | 系统监控与维护 | 数据备份恢复 | 自动备份、手动备份、数据恢复 | SEC-004 | 三 |
| 15 | 系统监控与维护 | **缓存管理** | 缓存配置、**缓存清理**、缓存性能优化 | **AUTHZ-004**（缓存失效）、`/authz/cache/stats` | 二 |
| 16 | 集成与接口管理 | 接口管理 | 接口配置、接口测试、接口文档、**调用监控** | SEC-005、OpenAPI 契约文件 | 三（可选） |
| 17 | 集成与接口管理 | **第三方系统集成** | 与 OA、HR、财务、项目、监控等系统**集成对接** | **AUTHZ-001~003**（对外授权接口）、IDM-009 | 一 |
| 18 | 集成与接口管理 | 数据同步管理 | 系统间数据同步配置、同步任务管理、同步状态监控 | IDM-009、ORG-005（`sys_import_task`） | 一 |
| 19 | 安全管理 | 密码策略管理 | 密码复杂度、有效期、历史密码、**登录失败锁定策略** | **SEC-001**、S-1、S-4 | 二 |
| 20 | 安全管理 | 双因素认证管理 | 短信、邮箱、动态令牌双因素认证 | ❌ **教师明确不要求实现** | — |
| 21 | 安全管理 | **数据安全管理** | 敏感数据**加密**、**脱敏**、访问管控、数据防泄漏 | **SEC-002**、S-10、S-11 | 一（加密脱敏）/ 二（防篡改） |

**覆盖情况**：21 个功能点中，**20 个纳入需求**（其中迭代一 11 个、迭代二 6 个、迭代三 3 个），1 个（双因素认证）经老师明确不要求而排除。

**几条值得在答辩时指出的对应关系**：

| 观察 | 说明 |
|---|---|
| 清单第 3 条写着「**分级**配置」与「权限**继承**设置」 | 这两个词直接对应 RBAC1 的角色继承与本系统的分级管理。老师的需求数据里早已埋好了 RBAC1 与 ARBAC 的要求 |
| 清单第 2、3、6 条三次提到「**数据权限**」 | 这与「查看本人」那 6 行互相印证：数据范围不是我们自行添加的功能，而是清单反复要求的能力 |
| 清单第 5 条明确「与 **HR 系统**同步」 | 印证了会议中「hr 系统来调用 api 生成用户」的说法，用户不是在本系统里手工创建的 |
| 清单第 13 条明确「**接口响应时间**监控」 | 与「高负载大并发」呼应，说明性能指标是需要被观测和展示的，而不只是压测一次 |
| 清单第 15 条单列「**缓存管理**」 | 说明缓存不是我们为了性能自行引入的实现细节，而是清单认可的系统能力——这为缓存失效设计（DP-2）提供了需求依据 |
